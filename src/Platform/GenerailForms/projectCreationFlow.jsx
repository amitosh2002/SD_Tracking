...
import { checkValidPartnerCodeAction, createProject } from '../../Redux/Actions/PlatformActions.js/projectsActions';
...
export default function ProjectCreationPage() {
  ...
  const [validPartnerCode, setValidPartnerCode] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!formData.partnerCode || formData.partnerCode.length < 3) {
    ...
    const debounceTimer = setTimeout(async () => {
      try {
        const response = await dispatch(checkValidPartnerCodeAction(formData.partnerCode));
        
        // Action returns res.data directly
        ...
        console.error("Partner code validation failed:", error);
        setValidPartnerCode(false);
      } finally {
        setIsValidating(false);
      }
  ...
  const validateForm = () => {
    ...
    } else if (!validPartnerCode) {
      newErrors.partnerCode = 'A valid and verified partner code is required';
    }
          ...
          <div className="form-section">
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <div className="input-with-icon">
                  <input
                    ...
                    className={`form-input ${
                      errors.partnerCode || (!validPartnerCode && !isValidating && formData.partnerCode.length >= 3) 
                        ? 'form-input--error' 
                        : (validPartnerCode ? 'form-input--success' : '')
                    }`}
                    placeholder="Enter unique partner code"
                  />
                  <div className="input-status-icon">
                    {isValidating && <Loader2 className="spinner" size={16} />}
                    {!isValidating && validPartnerCode && <CheckCircle className="text-success" size={16} />}
                  </div>
                </div>
                <div className="help-text">
                   <HelpCircle size={12} />
                ...
                {(errors.partnerCode || (!isValidating && !validPartnerCode && formData.partnerCode.length >= 3)) && (
                  <span className="form-error">
                    {errors.partnerCode || "Invalid or already taken partner code"}
                  </span>
                )}
              </div>
